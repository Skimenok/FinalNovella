...
const scenes = {
  ...
  level2_task1: {
    ...
    minigame: {
      ...
      data: [
        {
          q: "Я — пустота в коде, что стирает следы переменных, как забытые воспоминания. Новички видят меня как ошибку, профи — как способ обозначить отсутствие. Что я такое?",
          a: ["undefined", "null"],
          hint: "Используй меня, чтобы обозначить отсутствие значения.",
        },
        {
          q: "Как уборщик в терминале, я сметаю все логи и ошибки, оставляя чистый лист для новых идей.",
          a: ["console.clear()", "clear()", "console.clear", "clear"],
          hint: "Метод объекта console, который очищает вывод в браузерной консоли.",
        },
        {
          q: "Как метла для стилей, я сбрасываю все дефолты, очищая холст от браузерных 'призраков'.",
          a: ["reset.css", "reset"],
          hint: "Название CSS-файла или подхода, сбрасывающего стандартные стили браузера.",
        },
        {
          q: "Я стираю связь между элементом и действием. Удаляю того, кто слушал, освобождая память и предотвращая утечки. Как будто обрываю старые привязанности, даря элементу полную свободу. Кто я?",
          a: [
            "removeEventListener",
            "removeEventListener()",
          ...
          hint: "Метод для снятия обработчика события, противоположный addEventListener.",
        },
        {
          q: "Ты зовёшь меня — и я стираю всё внутри. Контент исчезает, будто и не был создан. Хоть я и свойство, но в этот миг я почти пустота. Кто я?",
          a: ["innerHTML", "innerHTML = ''", "element.innerHTML = ''", "inner"],
          hint: "JS-свойство, позволяющее получить или заменить HTML-содержимое элемента.",
        },
      ],
  ...
  level2_task2: {
    ...
    minigame: {
      ...
      data: [
        {
          q: "Я меняю форму элементов на странице, поворачиваю, растягиваю или сдвигаю их, как алхимик, превращающий воспоминания в новые видения. В CSS я — ключ к анимациям и 3D-эффектам.",
          a: [
            "transform",
            "CSS transform",
            "transform: rotate()",
          ...
          hint: "Свойство CSS для геометрических преобразований элементов, таких как поворот, масштаб или перемещение.",
        },
        {
          q: "В JavaScript я беру массив и превращаю каждый элемент в нечто новое, создавая свежий список, словно перерабатывая старые мысли в инновационные идеи для большей эффективности.",
          a: ["array.map()", "map()", "map"],
          hint: "Метод массива, который применяет функцию к каждому элементу и возвращает новый массив с результатами.",
        },
        {
          q: "В CSS я плавно меняю свойства со временем, превращая статичный элемент в динамичный, словно эволюционируя воспоминания в поток сознания.",
          a: ["transition", "CSS transition"],
          hint: "Свойство CSS для анимированных переходов между состояниями, такими как hover или active.",
        },
        {
          q: "Я превращаю числа в строки, позволяя им рассказывать истории. В коде я делаю числа словами, словно художник рисует цифры кистью. Кто я?.",
          a: ["String", "String()"],
          hint: "Можно использовать для конкатенации",
        },
        {
          q: "Я могу превратить любое значение в истину или ложь. С моей помощью проверка становится ясной, как день. Новичок видит меня как оператор, профи — как быстрый способ булева преобразования. Кто я?",
          a: ["Boolean", "Boolean()", "!!value", "!!"],
          hint: "Используется для приведения значения к true/false",
        },
      ],
      next: "act2_level3_start",
    },
